...
namespace nc
{
	...
	void Renderer::CreateWindow(const std::string& title, int width, int height)
	{
		...
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_COMPATIBILITY); // currently using old sdl_gl, modern is core?

		SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
		SDL_GL_SetAttribute(SDL_GL_ACCELERATED_VISUAL, 1); //for using video card / hardware

		SDL_GL_SetSwapInterval(1); //vsync

		m_context = SDL_GL_CreateContext(m_window);
		gladLoadGL(); //modern openGL functions
	...
	{
		glDepthMask(GL_TRUE);
		glClearColor(color.r, color.g, color.b, 1); //rgba
